...
void solve ()
{
    ...
    for(int i=0; i<N; i++)
        cin >> A[i];
    int  GDC_E=A[0], GDC_O = A[1];
    for(int i=0; i<N; i++)
    {
        if (i%2==0) GDC_E = __gcd(GDC_E, A[i]);
        else GDC_O = __gcd(GDC_O, A[i]);
    }
    bool oddbyeven = false, evenbyodd = false;
    for(int i=0; i<N; i+=2)
    {
        if(A[i]%GDC_O==0)
        {
    ...
    if(oddbyeven == true && evenbyodd == true) cout << 0;
    else if(oddbyeven == true) cout << GDC_O;
    else cout << GDC_E;
    cout << endl;
    return;
}
