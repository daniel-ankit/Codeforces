...
{
    if (start == end) // L aur R jab same hoga 
    {
        OCC[{start, end}] = start;
        return;
    }
    for (int i=start; i<=end; i++)  // L aur R jab same nahi hoga
    {
        if (i == start)
        {
            if (MAP[{i + 1, end}])
        ...
        else if (MAP[{start, i - 1}] && MAP[{i + 1, end}]) // Dono ke liye (Final)
        {
            OCC[{start, end}] = i;
            move(start, i-1);
            move(i+1, end);
...
void solve()
{
    ...
    {
        cin >> A[i].FF >> A[i].SS;
        MAP[{A[i].FF, A[i].SS}] = true;
    }
    move(start, end);
    for (auto itr : MAP)
    {
        if (OCC[itr.first])
            cout << (itr.FF).FF << " " << (itr.FF).SS << " " << OCC[itr.FF] << endl;
    }
    cout << endl;
}
