...
int32_t main()
{
    ...
    cin >> S;
    int in=0;
    if(S[0]=='h') cout << S[0] << S[1] << S[2] << S[3], in=4;
    else cout << S[0] << S[1] << S[2], in=3;
    cout << "://";
    for (int i=S.size()-3 ;i>=in; i--)
    {
        if (S[i] == 'r' && S[i+1] == 'u')
        {
            cout << S.substr(in, i-in) + ".ru/" + S.substr(i+2) << endl;
            return 0;
        }
    }
    cout << S.substr(in, S.size()-2-in) + ".ru" << endl;
    return 0;
}