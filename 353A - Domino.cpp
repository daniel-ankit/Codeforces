...
int32_t main()
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        S_UP += A[i].FF;
        S_DOWN += A[i].SS;
        if(oh_yeah==false && A[i].FF%2!=A[i].SS%2) oh_yeah = true;
    }
    if(S_UP%2==0 && S_DOWN%2==0) cout << 0;
    else if(S_UP%2 != S_DOWN%2) cout << -1;
    else if(S_DOWN%2==1 && S_UP%2==1)
        (!oh_yeah) ? (cout << -1) : (cout << 1);
    else cout << -1;
    return 0;
}