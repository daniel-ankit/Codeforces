...
bool ispalin(vector<int> a){ int l = 0; int r = a.size()-1; while(l < r) { if(a[l] != a[r]) return false; l++; r--; } return true; }
...
void solve()
{
    ...
    for(int i=0; i<N ;i++)
    {
        if(A[i] != A[LOW]) A1.PB(A[i]);
        if(A[i] != A[HIGH]) A2.PB(A[i]);
    }
    (ispalin(A1) || ispalin(A2)) ? (cout << "YES" << endl) : (cout << "NO" << endl);
}
