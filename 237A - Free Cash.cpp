...
int maxfreq(vector<int> V) { int C=1, MAX=0; SORT(V); int pivot = V[0]; for(int i=1; i<V.size(); i++) { if(V[i]!=pivot) { pivot = V[i]; C=0; } C++; MAX = max(MAX, C); } cout << MAX; }

int32_t main()
{
    Radhe Krishna
    int N, H=0, M=0, C=1, MAX=0;
    VI R;
    cin >> N;
    for(int i=0; i<N; i++)
    {
