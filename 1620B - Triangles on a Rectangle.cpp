...
int32_t main()
{
    ...
    TEST
    {
        ...
        if ((bottom[b - 1] - bottom[0]) > (top[t - 1] - top[0]))
            largex = bottom[b - 1] - bottom[0];
        else largex = top[t - 1] - top[0];
        if ((left[l - 1] - left[0]) > (right[r - 1] - right[0]))
            largey = left[l - 1] - left[0];
        else largey = (right[r - 1] - right[0]);
            cout << max(largex * h, largey * w) << "\n";
    }
    return 0;
}