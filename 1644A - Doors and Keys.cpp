...
int32_t main()
{
    ...
    TEST
    {
        string S;
        cin >> S;
        bool rbeforeR = 0, bbeforeB = 0, gbeforeG = 0;
        if(S.find('g')<S.find('G')) gbeforeG = 1;
        if(S.find('r')<S.find('R')) rbeforeR = 1;
        if(S.find('b')<S.find('B')) bbeforeB = 1;
        if(rbeforeR && bbeforeB && gbeforeG) cout << "YES" << endl;
        else cout << "NO" << endl;
    }
    return 0;
}